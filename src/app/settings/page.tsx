  ...
  return (
    <main className="scanlines min-h-screen bg-black text-white font-mono selection:bg-[#ff7a00] selection:text-black pb-20">
      ...
      <nav className="sticky top-0 z-40 flex items-center justify-between border-b border-[#333] bg-black/90 px-6 py-4 backdrop-blur-sm">
        ...
        <div className="text-[#ff7a00] font-black tracking-widest text-xl crt-flicker">
          SYS_CONFIG
        </div>
      </nav>
      ...
      <div className="mx-auto max-w-4xl p-6 md:p-8 space-y-12 mt-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          ...
          <section className="space-y-6">
            ...
            <form onSubmit={handleAddWatchlist} className="flex gap-2">
              <input 
                ...
                className="flex-1 bg-black border border-[#333] px-4 py-2 text-white focus:outline-none focus:border-[#ff7a00] focus:ring-1 focus:ring-[#ff7a00] uppercase placeholder:text-gray-700 transition-colors"
                maxLength={10}
              />
              <button 
                type="submit"
                className="bg-[#111] border border-[#333] px-4 py-2 text-white hover:border-[#ff7a00] hover:text-[#ff7a00] transition-colors"
              >
            ...
            <div className="border border-[#333] bg-[#050505] p-4 min-h-[200px]">
              ...
              <div className="flex flex-wrap gap-3">
                {watchlist.map((coin) => (
                  <div key={coin} className="group flex items-center gap-2 border border-[#ff7a00] bg-[#110000] pl-3 pr-1 py-1">
                    <span className="font-bold text-white">{coin}</span>
                    <button 
                      onClick={() => removeWatchlist(coin)}
                      className="p-1 text-gray-500 hover:text-[#ff7a00] transition-colors"
                      title="Remove target"
                    >
          ...
          <section className="space-y-6">
            ...
            <div className="border border-[#333] bg-black p-4 space-y-4 relative overflow-hidden group">
              <div className="text-xs text-[#ff7a00] mb-2 font-bold">// NEW_ALERT_RULE</div>
              
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  <label className="text-[10px] text-gray-500">TARGET ASSET</label>
                  <select className="w-full bg-black border border-[#333] p-2 text-sm text-white focus:border-[#ff7a00] outline-none appearance-none cursor-pointer">
              ...
              <button className="w-full border border-[#ff7a00] bg-[#110000] text-[#ff7a00] py-2 text-sm font-bold hover:bg-[#ff7a00] hover:text-black transition-colors">
            ...
            <div className="space-y-3">
              ...
              {alerts.map((alert) => (
                <div key={alert.id} className="flex items-start justify-between border-l-2 border-[#ff7a00] bg-[#0a0a0a] p-3">
                  <div>
                  ...
                  <button className="text-gray-600 hover:text-[#ff7a00] transition-colors">
        ...
        <div className="pt-12 border-t border-[#222] flex items-center justify-between text-xs text-gray-600">
          ...
          <button className="text-[#ff7a00] hover:underline underline-offset-4">
            [ CLEAR SYSTEM CACHE ]
          </button>
        </div>
